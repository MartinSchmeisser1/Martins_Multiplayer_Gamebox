...
public class SpaceShip2 : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if ((Input.GetAxis("Fire1_Trigger_Controller2") > 0.8) && shotTimer >= timeBetweenShots)
        {
            // Fire a shot
            Shooting();
    ...
    {
        // actual movement
        float horizontalInput = Input.GetAxis("Horizontal_Controller2");
        float verticalInput = Input.GetAxis("Vertical_Controller2");
        float rotationInput = Input.GetAxis("Rotation_Controller2");
    ...
    private void OnTriggerEnter(Collider other)
    {
        ...
        {
            if (GameManagerSpaceShooter.instance.shieldPowerupStatus[1])
            {
                GameManagerSpaceShooter.instance.shieldPowerupStatus[1] = false;
            }
            else
            {
                Destroy(gameObject);
