...
public class SpaceShip1 : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        // Shooting logic
        timeBetweenShots = GameManagerSpaceShooter.instance.attackspeedPowerupStatus[0];
        shotTimer += Time.deltaTime;
        if ((Input.GetButton("Fire1") || Input.GetAxis("Fire1_Trigger") > 0.8) && shotTimer >= timeBetweenShots)
        {
            // Fire a shot
            Shooting();
    ...
    void Shooting()
    {
        int numberOfLasers = GameManagerSpaceShooter.instance.multiattackPowerupStatus[0] + 1;
    ...
    private void OnTriggerEnter(Collider other)
    {
        ...
        {
            if (GameManagerSpaceShooter.instance.shieldPowerupStatus[0])
            {
                GameManagerSpaceShooter.instance.shieldPowerupStatus[0] = false;
            }
            else
            {
                Destroy(gameObject);
